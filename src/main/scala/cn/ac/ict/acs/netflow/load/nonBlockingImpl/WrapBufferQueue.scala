...
class WrapBufferQueue(
  ...
  private val bufferQueue = new ConcurrentLinkedQueue[ByteBuffer]()
  ...
  */
  ...
  // maybe should sleep when the queue is empty.
  def take(): ShareableBuffer = {
    var value = bufferQueue.poll()
    while(value == null){
      logInfo("occur one null in queue")
       Thread.sleep(1000)
      value = bufferQueue.poll()
    }
    val data = ShareableBuffer(value)
    dequeueCount.addAndGet(data.buffer.capacity())
    _currentPacket = data
    lock.synchronized {
      lock.notify()
  ...
  // Note: doesn't use checkThreshold Func, one reason is size() cost much
  // another is 麻烦，不便于测试
  // TODO: use checkThreshold()
  // 12.25 Arvin
  def put(byteBuffer: ByteBuffer): Unit = {
