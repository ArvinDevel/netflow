 ...
 */
package cn.ac.ict.acs.netflow.load.nonBlockingImpl
...
import cn.ac.ict.acs.netflow.JobMessages.{AllLoadersAvailable, GetAllLoaders, LoadInfo}
import cn.ac.ict.acs.netflow._
import cn.ac.ict.acs.netflow.ha.{LeaderElectable, LeaderElectionAgent, MonarchyLeaderAgent}
import cn.ac.ict.acs.netflow.load.master.CommandSet.CmdStruct
import cn.ac.ict.acs.netflow.load.master.LoadMasterMessages.CheckForWorkerTimeOut
import cn.ac.ict.acs.netflow.load.master._
...
class LoadMaster(masterHost: String, masterPort: Int, webUiPort: Int, val conf: NetFlowConf)
  ...
  // to let the compile pass, comment this line, cause it cause unpass.
  // 12.25 Arvin
...
//    masterMetricsSystem.start()
  ...
  // to send loadworker Ip to receiver,
  // 1) when receiver register early, while addNewWorker, send it to all receiver
  // 2) when loadWorker register early, while assignWorkers, in selectSuitableWorkers method,
  // let available Workers contain all the necessary workers.
  // Arvin
...
//          val collectorIP = if (waitQueue.contains(workerIP)) workerIP else waitQueue.head
          ...
          waitQueue.foreach(collectorIP =>
          loadServer.collector2Socket.get(collectorIP) match {
            case Some(socket) =>
              if (socket.isConnected) {
                // Now question is why the write method return 0? why fail
                // yeah, return len, and cmd is ByteBUffer,
                // maybe like iterator, after using is null.(wrong, ByteBuffer
                // has position pointer)
                // Arvin
                // after read data in Buffer, flip(); after write from Buffer
                // wantta use again, rewind();
                cmd.rewind()
                logDebug(cmd.toString)// luxuriously at all.! 1500 bytes
                ...
                // what an unexpected return Arvin
//                return
              } else {
                waitQueue.remove(collectorIP)
