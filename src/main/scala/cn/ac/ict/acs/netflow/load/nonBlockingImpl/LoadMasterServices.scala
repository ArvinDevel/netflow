...
//  private val ipToChannel = mutable.HashMap.empty[String, SocketChannel]
  ...
  // contain ip+port(tag) and channel
  // 12.24 Arvin
  private val tag2Channel = mutable.HashMap.empty[String,SocketChannel]
  ...
  // changed by Arvin 12.24
  def collector2Socket = tag2Channel // ipToChannel
  ...
  // add getTag(ip + port) method 12.24 Arvin
  // Note: socketChannel has getRemoteAddress and getLocalAddress!
  // Another: let LoadMaster send loadworker ip to receiver!
  private def getRemoteTag(socketChannel: SocketChannel): String = {
    socketChannel.getRemoteAddress.asInstanceOf[InetSocketAddress]
...
//    logInfo(s"Open connection from ${_remoteIP}.")
    ...
    val _remoteTag = getRemoteTag(socketChannel)
    tag2Channel(_remoteTag) = socketChannel
    logInfo(s"Open connection from ${_remoteTag}.")
...
//    master ! DeleReceiver(collectorIP)
    ...
    val collectorTag = getRemoteTag(c.asInstanceOf[SocketChannel])
    tag2Channel -= collectorTag
    master ! DeleReceiver(collectorTag)
  }
...
//      logDebug(s"Tell master request a worker ip for $remoteIP")
      // cause remoteIp was put HashSet, so the same Ip can't connect multi connection. 12.24 Arvin
      // still should change collecotr2socket(ipToChannel)which is in this class
      //  wattingQueue in LoadMaster
      val remoteTag = getRemoteTag(curChannel)
      logDebug(s"Tell master request a worker ip for $remoteTag")

      master ! RequestWorker(remoteTag)
    }
  }
